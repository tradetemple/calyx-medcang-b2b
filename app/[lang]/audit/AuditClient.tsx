  ...
  return (
    <tr className="border-b border-slate-800 hover:bg-slate-900/50 transition-colors font-mono text-[11px]">
      ...
      <td className="py-2 px-3">
        <span className="bg-slate-800 text-emerald-500 px-2 py-0.5 rounded-none border border-emerald-500/20 font-bold text-[9px] tracking-tighter">
          {event.actor}
        </span>
      </td>
...
export default function AuditVaultPage(dict: AuditVaultProps) {
  ...
  // Merge logs with initial logs if logs are empty (for first-time use/display)
  const displayLogs = logs.length > 0 ? logs : (_initialLogs || []);

  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 font-mono p-4 md:p-8 selection:bg-emerald-500/30">
      <div className="max-w-7xl mx-auto space-y-6">
        
        {/* Header Section */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-800 pb-6">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-emerald-500 text-xs font-bold tracking-[0.2em] uppercase">
              <ShieldCheck className="w-4 h-4" />
              <span>{t.header.title}</span>
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse ml-2" />
              <span className="text-[10px] text-emerald-500/50 ml-1">{t.header.live}</span>
            </div>
            <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tighter uppercase flex items-center gap-3">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden md:block">
              <div className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">{t.header.nodeIdentity}</div>
              <div className="text-xs text-slate-300 font-bold">{t.header.mockId}</div>
            </div>
            <button 
              onClick={clearLogs}
              className="px-3 py-1 border border-slate-700 text-slate-500 text-[10px] uppercase font-bold hover:bg-slate-800 hover:text-slate-300 transition-colors"
            >
              {t.header.flushLogs}
            </button>
          </div>
        ...
        <div className="bg-slate-900/30 border border-slate-800 rounded-sm overflow-hidden backdrop-blur-sm shadow-2xl">
          ...
          <div className="overflow-x-auto min-h-[400px]">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-900/50 text-slate-500 text-[10px] uppercase tracking-widest font-bold border-b border-slate-800 sticky top-0">
                <tr>
        ...
        {/* Technical Footer */}
        <div className="flex flex-col md:flex-row justify-between gap-4 text-[9px] text-slate-600 uppercase tracking-widest font-bold pt-4">
